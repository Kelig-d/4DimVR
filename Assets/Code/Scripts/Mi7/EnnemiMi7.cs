...
public class EnnemiMi7 : MonoBehaviour
{
    private bool attackMod;
    public GameObject Lumiere;
    Vector3 positionOnde;
    public bool presenceOndeLumineuse;
    public GameObject Onde_Lumineuse;
    private float minHeight = -10f; // Hauteur minimale
    private float maxHeight = 45f; // Hauteur maximale
    private bool goingUp = true; // Direction actuelle
    ...
    void Start()
    {
        Lumiere = transform.Find("Onde_Lumineuse/Lumiere").gameObject;
        attackMod = false;
        presenceOndeLumineuse = true;
        positionOnde = Onde_Lumineuse.transform.position;
        ...
        while (!isDead)
        {
            if(animator.GetBool("isTakingDamages") == false)
            {
                ...
                if (Target)
                {
                    ...
                    Lumiere.GetComponent<Light>().color = Color.red;
                    attackMod = true;
                ...
                else
                {
                    ...
                    sonEnnemi.SetActive(false);
                    Lumiere.GetComponent<Light>().color = Color.white;
    ...
    {
        isDead = true;
        presenceOndeLumineuse = false; // Arret du déplacemeent de l'Onde lumineuse
        positionOnde.y = minHeight; // Réinitialisation de la position de l'Onde lumineuse
        ennemi.destination = ennemi.transform.position;
    ...
    private void deplacementOndeLumineuse(bool actif, bool attackMod){
        if(actif || (!actif && goingUp)){
            ...
            if (goingUp)
            {
                positionOnde.x = ennemi.transform.position.x;
                positionOnde.z = ennemi.transform.position.z;
                if(attackMod){
                    positionOnde.y += 0.51f;
                }else{
                    positionOnde.y += 0.15f;
                }
                

                // Si l'objet atteint la hauteur maximale
                if (positionOnde.y >= maxHeight)
                {
                    positionOnde.y = maxHeight;
                    goingUp = false; // Change la direction
                }
            }
            else
            {
                positionOnde.y = minHeight; // Réinitialise la hauteur
                goingUp = true; // Repart vers le haut
            }
            //Debug.Log(positionOnde);
            // Applique la nouvelle position
            Onde_Lumineuse.transform.position = positionOnde;
        }
    }
}